...
public:
  ...
  template <typename T> void doSort(std::vector<T> &toSort) const {
...
private:
  ...
  // ------------------------------------------------------------------------------
  template <typename T> std::vector<T> mergeSort(std::vector<T> &toSort) const {
    if (toSort.size() == 1) {
      return toSort;
    }
  ...
  getMiddleAndStopIndex(const std::vector<int> &sizes) const {
    ...
    for (auto index = 1u; index < sizes.size(); ++index) {
      ...
      middleAndStopIndex.emplace_back(static_cast<size_t>(middleIndex),
  ...
  void setSizes(const mpi::communicator &world, std::vector<int> &sizes,
    ...
    const auto numberOfElementsToProcess =
        totalNumberOfElements / numberOfRanks;
    const auto elementsToAddToMasterRank =
        totalNumberOfElements % numberOfRanks;
